...
public static  String paging4(int totalRecord, int nowPage, int recordPerPage, String filenm){
	...
	public static  String bbspaging(int totalRecord, int nowPage, int recordPerPage, String filenm,String s_id){
	    ...
	    int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동
	    if (nowGrp >= 2){
	      str.append("[<A href='./"+filenm+"?s_id="+s_id+"&nowPage="+_nowPage+"'>이전</A>]");
	    }
	    ...
	    for(int i=startPage; i<=endPage; i++){
	      ...
	      if (nowPage == i){ // 현재 페이지이면 강조 효과
	        ...
	        str.append("<A href='./"+filenm+"?s_id="+s_id+"&nowPage="+_nowPage+"'>"+i+"</A>&nbsp;");
	      }
	    ...
	    _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동
	    if (nowGrp < totalGrp){
	      str.append("[<A href='./"+filenm+"?s_id="+s_id+"&nowPage="+_nowPage+"'>다음</A>]");
	    }
	    str.append("</DIV>");
	...
	public static  String qnapaging(int totalRecord, int nowPage, int recordPerPage, String filenm,String id){
	    int pagePerBlock = 3; // 블럭당 페이지 수
	    int totalPage = (int)(Math.ceil((double)totalRecord/recordPerPage)); // 전체 페이지 
	    int totalGrp = (int)(Math.ceil((double)totalPage/pagePerBlock));// 전체 그룹
	    int nowGrp = (int)(Math.ceil((double)nowPage/pagePerBlock));    // 현재 그룹
	    ...
	    int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동
	    if (nowGrp >= 2){
	      str.append("[<A href='./"+filenm+"?nowPage="+_nowPage+"&id="+id+"'>이전</A>]");
	    }
	    ...
	    for(int i=startPage; i<=endPage; i++){
	      ...
	      if (nowPage == i){ // 현재 페이지이면 강조 효과
	        ...
	        str.append("<A href='./"+filenm+"?nowPage="+i+"&id="+id+"'>"+i+"</A>&nbsp;");
	      }
	    ...
	    _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동
	    if (nowGrp < totalGrp){
	      str.append("[<A href='./"+filenm+"?nowPage="+_nowPage+"&id="+id+"'>다음</A>]");
	    }
	    str.append("</DIV>");
